...
def train(args, train_dataset, valid_dataset, model, tokenizer, vteacher) -> Tuple[int, float]:
    ...
    if args.max_steps > 0:
        ...
        args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps) + 10
...
#         vteacher, optimizer = amp.initialize(vteacher, optimizer, opt_level=args.fp16_opt_level)
        from apex.parallel import DistributedDataParallel as DDP
        model = DDP(model)
#         vteacher = DDP(vteacher)
    else:
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[args.gpu], find_unused_parameters=True
        )
#         vteacher = torch.nn.parallel.DistributedDataParallel(
#             vteacher, device_ids=[args.gpu], find_unused_parameters=True
#         )
...
def evaluate(args, eval_dataset, model, tokenizer, vteacher, prefix="") -> Dict:
    torch.cuda.empty_cache() 
    ...
    for (tokens, item_ids) in tqdm(eval_dataloader, desc="Evaluating"):
        token_inputs, token_labels = mask_tokens(tokens, tokenizer, args)
        token_inputs = token_inputs.to(args.device)
        token_labels = token_labels.to(args.device)
...
#     model.load_state_dict(model_weights, strict=True)
    ...
    # Training
    if args.do_train:
        train(args, train_dataset, valid_dataset, model, tokenizer, vteacher)
